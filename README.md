...
MCP_HOST=127.0.0.1                         # Adresa pro naslouchani (vychozi 127.0.0.1; Docker obraz 0.0.0.0)
MCP_AUTH_TOKEN=                            # Bearer token pro HTTP auth — POVINNY, pokud MCP_HOST neni loopback
```
...
# Spusteni HTTP rezimu (jen lokalne na 127.0.0.1, bez tokenu)
MCP_TRANSPORT=http MCP_PORT=3000 npm start

# Spusteni HTTP rezimu pro sitovy pristup (token je povinny)
MCP_TRANSPORT=http MCP_HOST=0.0.0.0 MCP_AUTH_TOKEN=$(openssl rand -hex 32) npm start
...
2. Nastavte bezpecnostni token (povinne — bez nej Docker Compose ani kontejner nenastartuje):

```bash
cp .env.example .env
# Doplnte MONEY_S3_DOMAIN, CLIENT_ID, CLIENT_SECRET, APP_ID
# Nastavte MCP_AUTH_TOKEN (povinny):
...
HTTP rezim je **fail-closed**: server ve vychozim stavu nasloucha jen na `127.0.0.1` a do site se bez tokenu nevystavi.
...
- `MCP_HOST` urcuje adresu pro naslouchani — vychozi `127.0.0.1`, Docker obraz nastavuje `0.0.0.0`
- Nastaven `MCP_AUTH_TOKEN` → kazdý pozadavek na `/mcp` musi obsahovat hlavicku `Authorization: Bearer <token>`
- Nenastaven `MCP_AUTH_TOKEN` a `MCP_HOST` je loopback (`127.0.0.1`, `localhost`, `::1`) → `/mcp` je bez tokenu, ale prijima jen pozadavky s lokalnim `Host` / `Origin` (ochrana proti DNS rebinding utokum z prohlizece)
- Nenastaven `MCP_AUTH_TOKEN` a `MCP_HOST` neni loopback (napr. `0.0.0.0`) → **server odmitne nastartovat**
